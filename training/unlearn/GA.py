...
def get_training_config_GA(args):
    dataset_name = args.get('dataset_name', '').lower()
    net_name = args.get('net_name', '').lower()
    
    config = {
        'num_alternations': args.get('num_alternations', 8),
        'remove_epoch': 3,
        'retain_epoch': 1,
        'remove_lr_multiplier': 0.1,
    ...
    if dataset_name == 'sst5':
        config['num_alternations'] = args.get('num_alternations', 4)
        config['remove_lr_multiplier'] = 2.0
    elif dataset_name == 'mnli' and net_name == 'gpt2':
        config['num_alternations'] = args.get('num_alternations', 6)
    elif dataset_name == 'news20':
        config['remove_epoch'] = 3
        ...
        config['remove_lr_multiplier'] = 2.0
    elif dataset_name == 'cinic10' or dataset_name == 'tinyimagenet':
        config['remove_lr_multiplier'] = 0.05
    elif dataset_name == 'cifar100':
        config['remove_epoch'] = 1
        config['retain_epoch'] = 1
        config['num_alternations'] =20
        config['remove_lr_multiplier'] = 0.6
    ...
    if 'num_alternations' in args:
        config['num_alternations'] = args['num_alternations']
    if 'remove_epoch' in args:
        config['remove_epoch'] = args['remove_epoch']
    if 'retain_epoch' in args:
        config['retain_epoch'] = args['retain_epoch']
    if 'remove_lr_multiplier' in args:
        config['remove_lr_multiplier'] = args['remove_lr_multiplier']
    if 'forget_lr' in args:
        base_lr = args.get('lr', 0.001)
        config['remove_lr_multiplier'] = args['forget_lr'] / base_lr if base_lr > 0 else 0.1
    if 'retain_lr' in args:
        config['retain_lr'] = args['retain_lr']
    
    return config


def GA_train_on_remove_set(model, remove_loader, args, num_epochs=3, learning_rate=None, weight_decay=None):
    if learning_rate is None:
        lr_multiplier = args.get('remove_lr_multiplier', 0.1)
        learning_rate = args.get('forget_lr', args.get('lr', 0.001) * lr_multiplier)
    if weight_decay is None:
        weight_decay = args.get('remove_weight_decay', 5e-4)
    
    optimizer_ascent = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    criterion = nn.CrossEntropyLoss()
    
    model.train()
    for epoch in range(num_epochs):
        gradient_ascent(model, remove_loader, optimizer_ascent, criterion, args)


def GA_train_on_retain_set(model, retain_loader, args, num_epochs=1, learning_rate=None, weight_decay=None):
    if learning_rate is None:
        learning_rate = args.get('retain_lr', args.get('lr', 0.001))
    if weight_decay is None:
        weight_decay = args.get('retain_weight_decay', 5e-4)
    
    optimizer_remained = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    criterion = nn.CrossEntropyLoss()
...
def train_single_model_GA(args, train_dataset, test_dataset, num_classes, trial, total_unlearn_steps, is_shadow=False):
    ...
    training_config = get_training_config_GA(args)
    ...
    remove_lr_multiplier = training_config['remove_lr_multiplier']
    remove_lr = args.get('lr', 0.001) * remove_lr_multiplier
    ...
    print(f"    {model_prefix} ->   Remove epoch: {remove_epoch}, LR: {remove_lr} (multiplier: {remove_lr_multiplier}), Weight decay: {remove_weight_decay}")
        ...
        if not has_any_operation:
            ...
            for alt_iter in range(num_alternations):
                ...
                if remove_loader is not None:
                    GA_train_on_remove_set(current_model, remove_loader, args, 
                                          num_epochs=remove_epoch,
                                          learning_rate=remove_lr,
                                          weight_decay=remove_weight_decay)
                    print(
                        f"      {model_prefix} -> Remove unlearn (GA gradient ascent): {len(remove_indices)} samples, {remove_epoch} epochs")
                ...
                if retain_loader is not None:
                    GA_train_on_retain_set(current_model, retain_loader, args, 
                                         num_epochs=retain_epoch,
                                         learning_rate=retain_lr,
                                         weight_decay=retain_weight_decay)
                    print(
                        f"      {model_prefix} -> Retain train (GA normal): {len(retain_indices)} samples, {retain_epoch} epochs")
...
def continuous_update_finetune_GA(args):
