...
from .data_loader import (
    DataLoader,
    load_outputs_and_labels_for_mia_threshold,
    load_ppl_member_nonmember_scores,
    load_training_data,
)
...
def _median_threshold_from_train_values(tr_values: np.ndarray) -> float:
    if tr_values is None or len(tr_values) == 0:
        return 0.0
    sorted_values = np.sort(np.array(tr_values, dtype=np.float64))
...
def load_target_data_for_mia(
    ...
    _atk = target_args.get('attack_method', base_args.get('attack_method', '')) or ''
    if _atk in ('mia_threshold', 'TW_MIA'):
        num_insert_only = 50
        num_remove_only = 50
...
def run_mia_threshold_attack(
    ...
    # load_training_data pattern branch keys off mia_threshold / TW_MIA
    _am = shadow_args_copy.get('attack_method', '')
    if _am not in ('mia_threshold', 'TW_MIA'):
        shadow_args_copy['attack_method'] = 'mia_threshold'
    ...
    if use_ppl:
        ...
        s_tr_scores, _ = load_ppl_member_nonmember_scores(
            shadow_args_copy, shadow_indices, data_root=data_root
        )
        threshold_median = _median_threshold_from_train_values(s_tr_scores)
        ...
        if outputs.ndim != 3 or outputs.shape[-1] != 1:
            raise ValueError(
                f"PPL TW-MIA expects target outputs (N,T,1), got shape {outputs.shape}"
            )
        N, T, _ = outputs.shape
        pred = np.zeros((N, T), dtype=np.int64)
        for n in range(N):
            for t in range(T):
                ppl = float(outputs[n, t, 0])
                if not np.isfinite(ppl) or ppl <= 0:
                    pred[n, t] = 0
                else:
                    score = -np.log(max(ppl, 1e-30))
                    pred[n, t] = 1 if score >= threshold_median else 0
        ...
        predictions = {
            ...
            'ppl_threshold_train_median': threshold_median,
        }
    else:
